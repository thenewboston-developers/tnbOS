import {setLectureListPayload} from 'apps/University/payloads';
...
interface SetLectureListBlock {
  networkId: string;
  params: Lecture[];
  recipient: string;
}

const setLectureListBlock = async ({networkId, params, recipient}: SetLectureListBlock) => {
  const {
    system: {self},
  } = store.getState();
  ...
  const data: UnsignedBlock = {
    ...
    payload: setLectureListPayload(params),
    recipient,
...
export default setLectureListBlock;
