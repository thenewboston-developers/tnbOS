import {getLectureRecordPayload} from 'apps/University/payloads';
import {GetLectureRecordParams} from 'apps/University/types';
...
interface GetLectureRecordBlock {
  networkId: string;
  params: GetLectureRecordParams;
  recipient: string;
}

const getLectureRecordBlock = async ({networkId, params, recipient}: GetLectureRecordBlock) => {
  const {
    system: {self},
  } = store.getState();
  ...
  const data: UnsignedBlock = {
    ...
    payload: getLectureRecordPayload(params),
    recipient,
...
export default getLectureRecordBlock;
