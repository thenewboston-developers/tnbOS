import {getCourseListPayload} from 'apps/University/payloads';
...
interface GetCourseListBlock {
  networkId: string;
  params: string[];
  recipient: string;
}

const getCourseListBlock = async ({networkId, params, recipient}: GetCourseListBlock) => {
  const {
    system: {self},
  } = store.getState();
  ...
  const data: UnsignedBlock = {
    ...
    payload: getCourseListPayload(params),
    recipient,
...
export default getCourseListBlock;
