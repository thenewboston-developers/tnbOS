import {getProductListPayload} from 'apps/Shop/payloads';
...
interface GetProductListBlock {
  networkId: string;
  params: string[];
  recipient: string;
}

const getProductListBlock = async ({networkId, params, recipient}: GetProductListBlock) => {
  const {
    system: {self},
  } = store.getState();
  ...
  const data: UnsignedBlock = {
    ...
    payload: getProductListPayload(params),
    recipient,
...
export default getProductListBlock;
