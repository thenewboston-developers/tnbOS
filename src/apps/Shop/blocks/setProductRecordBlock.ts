import {setProductRecordPayload} from 'apps/Shop/payloads';
...
interface SetProductRecordBlock {
  networkId: string;
  params: ProductRecord;
  recipient: string;
}

const setProductRecordBlock = async ({networkId, params, recipient}: SetProductRecordBlock) => {
  const {
    system: {self},
  } = store.getState();
  ...
  const data: UnsignedBlock = {
    ...
    payload: setProductRecordPayload(params),
    recipient,
...
export default setProductRecordBlock;
