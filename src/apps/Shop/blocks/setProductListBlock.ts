import {setProductListPayload} from 'apps/Shop/payloads';
...
interface SetProductListBlock {
  networkId: string;
  params: Product[];
  recipient: string;
}

const setProductListBlock = async ({networkId, params, recipient}: SetProductListBlock) => {
  const {
    system: {self},
  } = store.getState();
  ...
  const data: UnsignedBlock = {
    ...
    payload: setProductListPayload(params),
    recipient,
...
export default setProductListBlock;
