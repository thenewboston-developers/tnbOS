...
import {SHOP_RECEIVING_ACCOUNTS} from 'apps/Shop/store/constants';
import {ReceivingAccount, ReceivingAccounts} from 'apps/Shop/types';
...
const receivingAccounts = createSlice({
  ...
  reducers: {
    setReceivingAccount: (state: ReceivingAccounts, {payload: receivingAccount}: PayloadAction<ReceivingAccount>) => {
      ...
      window.electron.ipc.send(IpcChannel.setStoreValue, {key: SHOP_RECEIVING_ACCOUNTS, state: current(state)});
    },
    setReceivingAccounts: setLocalAndStateReducer<ReceivingAccounts>(SHOP_RECEIVING_ACCOUNTS),
  },
});
