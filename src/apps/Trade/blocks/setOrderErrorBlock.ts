import setOrderErrorPayload from 'apps/Trade/payloads/setOrderErrorPayload';
import {OrderError} from 'apps/Trade/types';
import {UnsignedBlock} from 'shared/types';
import {CORE_TRANSACTION_FEE} from 'system/constants/protocol';
import {createBlock} from 'system/core/blocks';
import store from 'system/store';
import {signData} from 'system/utils/signing';

interface SetOrderErrorBlock {
  networkId: string;
  params: OrderError;
  recipient: string;
}

const setOrderErrorBlock = async ({networkId, params, recipient}: SetOrderErrorBlock) => {
  const {
    system: {self},
  } = store.getState();

  const data: UnsignedBlock = {
    amount: 0,
    id: crypto.randomUUID(),
    payload: setOrderErrorPayload(params),
    recipient,
    sender: self.accountNumber,
    transaction_fee: CORE_TRANSACTION_FEE,
  };

  const block = signData(data, self.signingKey);
  return createBlock(block, networkId);
};

export default setOrderErrorBlock;
