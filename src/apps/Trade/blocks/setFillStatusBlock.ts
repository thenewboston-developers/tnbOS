...
import {SetFillStatusParams} from 'apps/Trade/types';
...
const setFillStatusBlock = async ({networkId, params, recipient}: SetFillStatusBlock) => {
  const {
    system: {self},
  } = store.getState();
  ...
  const data: UnsignedBlock = {
    ...
    payload: setFillStatusPayload(params),
    recipient,
...
export default setFillStatusBlock;
