import {pongPayload} from 'apps/SpeedTest/payloads';
import {PongParams} from 'apps/SpeedTest/types';
...
const pongBlock = async ({networkId, params, recipient}: PongBlock) => {
  const {
    system: {self},
  } = store.getState();
